...
with old_table as
(
    select * from {{ref('conv_wbx_prc_ppv_forecast_exch_rate')}}  
...
base_dim  as (
    select * from {{ ref ('stg_f_wbx_prc_ppv_forecast_exch_rate') }}
...
old_model_conv as (
    select
        ...
        cast(substring(exch_rate_year,1,60) as text(60) ) as exch_rate_year  ,
        ...
        cast(exchange_rate as number(28,9) ) as exchange_rate  ,
...
base_dim_conv as (
    ...
    from base_dim 
    {% if check_table_exists( this.schema, this.table ) == 'True' %}
    where exch_rate_year||scenario 
        not in (
                select distinct exch_rate_year||scenario from {{ this }}
