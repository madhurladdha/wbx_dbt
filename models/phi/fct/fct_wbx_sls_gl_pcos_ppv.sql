{{
    config(
        ...
        tags=["sales", "gl", "ppv", "sales_gl_allocations"],
        unique_key='GL_DATE',
...
{% set account_category_list = "('PCOS VAR - FG','PCOS VAR- OTHER','PCOS VAR - PACKAGIN','PCOS VAR - COPACK','PCOS VAR - LABOUR')" %}
...
with
    ...
    ax_hist as 
    (
        select * from {{ ref('conv_fct_wbx_sls_gl_pcos_ppv') }} 
    ...
    (select gl_month,company_code,item_guid,product_class_code,source_item_identifier, sum(perc_product_class) perc_product_class 
    from allocbasis group by all),        --For when we have ONLY product class
    allocbasis_no_sku_no_productclass as (select distinct gl_month,company_code,item_guid, source_item_identifier,product_class_code,
    sum(perc_month) perc_month from allocbasis group by all),
    ...
    itm_ext_group as (
        select distinct item_guid, source_item_identifier, product_class_code
        from item_ext
    ),
    ...
    (
        select
            ...
            itm_ext.product_class_code as product_class,  -- Field should reference Financial Tags
            NVL(WOF.source_item_identifier, ITM_EXT.source_item_identifier) as source_item_identifier,  -- Field should reference Financial Tags
        ...
        inner join wo wof on wof.voucher = gl.reference2_txt 
        inner join itm_ext_group itm_ext on wof.source_item_identifier = itm_ext.source_item_identifier --gl.sku=itm_ext.source_item_identifier, valid sku
        where
             NVL(WOF.source_item_identifier, ITM_EXT.source_item_identifier) --PB:this condition not needed. since its inner join
            is not null
    ...
    (
        ...
        inner join dim_date_cte dt on dt.calendar_date = gl.gl_date
        left join
            itm_ext_group itm_ext on gl.sku = itm_ext.source_item_identifier
        inner join prod_class on gl.product_class = prod_class.product_class_code
        left join wo wof on wof.voucher = gl.reference2_txt--Just join on item
        inner join
            allocbasis_product_class allobasic  --valid trade type
            on gl.product_class = allobasic.product_class_code
        ...
        where
           ...
           and wof.source_item_identifier is null
            and prod_class.product_class_code is not null --this is not needed, since we are doing inner join above
        group by
            gl.source_system,
            gl.target_account_identifier,
            gl.account_guid,
    ...
    Scenario3 as (
        ...
        left join wo wof on wof.voucher = gl.reference2_txt
        inner join
            allocbasis_no_sku_no_productclass allobasic on allobasic.gl_month = date_trunc('month', gl.gl_date)
            and gl.document_company = allobasic.company_code
