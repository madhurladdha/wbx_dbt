...
/*
PCOS PPV History from the P&L History file data set for IBE.
...
/* PCOS PPV (Varance) Fields Mapping: as of 14-Mar-2024 there are 6 such fields for PPV.  This will be un-pivoted into the gl_pcos_ppv_fact structure.  Code is simply using the first
    such account that falls under the PCOS Variance Account Category.  Can only specify one.

PCOS_Var_Ingredients	510010
PCOS_Var_Packaging	510030
PCOS_Var_Labour	500900
PCOS_Var_Co-Packing	520030
PCOS_Var_Bought-In	500030
PCOS_Var_Other	520010
*/
...
with
    ...
    /* For each of the incoming fields, for PCOS PPV there are 6 applicable, the code is un-pivoting from columns to rows to fit in with the GL PCOV PPV Fact structure.
    ...
    */
    ...
    (
        ...
        from pl_hist
        unpivot 
            (txn_ledger_amt for txn_pivot_field in (txn_pcos_var_ingredients,txn_pcos_var_packaging,txn_pcos_var_labour,txn_pcos_var_co_packing,txn_pcos_var_bought_in,txn_pcos_var_other))
        unpivot
            (pcomp_ledger_amt for pcomp_pivot_field in (pcomp_pcos_var_ingredients,pcomp_pcos_var_packaging,pcomp_pcos_var_labour,pcomp_pcos_var_co_packing,pcomp_pcos_var_bought_in,pcomp_pcos_var_other))
        unpivot
            (phi_ledger_amt for phi_pivot_field in (phi_pcos_var_ingredients,phi_pcos_var_packaging,phi_pcos_var_labour,phi_pcos_var_co_packing,phi_pcos_var_bought_in,phi_pcos_var_other))
        unpivot
            (base_ledger_amt for base_pivot_field in (base_pcos_var_ingredients,base_pcos_var_packaging,base_pcos_var_labour,base_pcos_var_co_packing,base_pcos_var_bought_in,base_pcos_var_other))
    ),
    ...
    */
    ...
    (
        select * ,
            case 
                when hist.txn_pivot_field='TXN_PCOS_VAR_INGREDIENTS' then '510010'
                when hist.txn_pivot_field='TXN_PCOS_VAR_PACKAGING' then '510030'
                when hist.txn_pivot_field='TXN_PCOS_VAR_LABOUR' then '500900'
                when hist.txn_pivot_field='TXN_PCOS_VAR_CO_PACKING' then '520030'
                when hist.txn_pivot_field='TXN_PCOS_VAR_BOUGHT_IN' then '500030'  --Might need changing, but provided by DLG on 3/15/24.
                when hist.txn_pivot_field='TXN_PCOS_VAR_OTHER' then '520010'
