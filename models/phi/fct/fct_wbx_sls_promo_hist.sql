...
-- depends_on: {{ ref('fct_wbx_sls_promo') }}
...
{{
    config(
        ...
        DELETE FROM {{ref('fct_wbx_sls_promo')}} WHERE SNAPSHOT_DATE NOT IN (SELECT DISTINCT SNAPSHOT_DATE FROM {{ref('dim_wbx_fc_snapshot')}} ) 
        OR SNAPSHOT_DATE < DATEADD(WEEK,-13,CURRENT_DATE) 
		{% endif %}  
        """
...
with hist_fct as
(
    select * from {{ref('conv_fct_wbx_sls_promo_hist')}} 
