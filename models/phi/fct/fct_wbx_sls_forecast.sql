...
with old_table as
(
    select * from {{ref('conv_fct_wbx_sls_forecast')}}  
...
base_fct  as (
    select * from {{ref('int_f_wtx_sls_forecast')}}
...
(
    ...
    cast({{ dbt_utils.surrogate_key(["source_system", "scen_code"]) }} as text(255) ) as scenario_guid  ,
