...
{{
    config(
        ...
        tags=["sales", "promotion", "sls_promo"],
        ...
        pre_hook="""
                 ...
                 DELETE FROM {{this}} 
                 WHERE TO_DATE(SNAPSHOT_DATE) 
                 =(select sn.snapshot_Date  
                   from {{ ref('stg_d_wtx_lkp_snapshot_date') }} sn);
                 {% endif %}  
                 """,
        post_hook="""
                  DELETE FROM {{this}} 
                  WHERE SNAPSHOT_DATE < (select DATEADD(Day ,-35,snapshot_Date) 
                                         from {{ ref('stg_d_wtx_lkp_snapshot_date') }}) 
                  AND
                  SNAPSHOT_DATE NOT IN 
                  (SELECT SNAPSHOT_DATE FROM {{ ref('dim_wbx_fc_snapshot') }}
                  WHERE SNAPSHOT_TYPE IN ('WEEK_END','MONTH_END') ) ;
                  """,
    )
}}
...
with
    old_table as (
        select *
        from {{ ref("conv_fct_wbx_sls_promo") }}
    ...
    new_fct as (
        select
            ...
            cast(unique_key as text(16777216)) as unique_key
        from new_table
