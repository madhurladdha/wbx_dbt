{{
    config(
        ...
        tags=["wbx", "manufacturing", "work order"],
...
with
    source as (select * from {{ ref("int_f_wbx_mfg_wo") }}),
    final as (
        select
            *,
            row_number() over (
                partition by unique_key order by 1
            ) rownum
        from source
    )
...
select
    ...
    --cast(wo_produced_fact_guid as text(255)) as wo_fact_guid,

    cast(
        substring(source_order_type_code, 1, 255) as text(255)
    ...
    cast(
        substring(wo_creator_add_number, 1, 255) as text(255)
    ) as wo_creator_add_number,

    cast(substring(manager_add_number, 1, 255) as text(255)) as manager_add_number,

    cast(
        substring(supervisor_add_number, 1, 255) as text(255)
    ...
    cast(snapshot_date as timestamp_ntz(9)) as scheduled_snapshot_date,

    cast(snapshot_version as number(20, 0)) as scheduled_snapshot_version,

    cast(planned_completion_date as date) as orig_planned_completion_date,

    cast(planned_start_date as date) as orig_planned_start_date,
