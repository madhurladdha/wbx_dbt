...
with
    ...
    /* Pick Actuals prior to current week and forecast after current week */
    uber_cte as (
        select
            source_system,
    ...
    pre_processing as (
        select
            ...
            sum(
                ub.cy_shipped_ca_quantity * itm_ext.consumer_units
            ) as "CY Despatched Consumer Unit Quantity",
            sum(nvl(ub.cy_shipped_kg_quantity, 0)) as "Cy Despatched Kg Quantity",
            sum(
                ub.cy_shipped_ca_quantity * itm_ext.consumer_units_in_trade_units
            ...
            sum(
                ub.ly_shipped_ca_quantity * itm_ext.consumer_units
            ) as "LY Despatched Consumer Unit Quantity",
            sum(nvl(ub.ly_shipped_kg_quantity, 0)) as "Ly Despatched Kg Quantity",
            sum(
                ub.ly_shipped_ca_quantity * itm_ext.consumer_units_in_trade_units
            ...
            sum(
                nvl(ub.fcf_tot_vol_ca * itm_ext.consumer_units, 0)
...
select
    currentweek_unique_key as cw_unique_key,
    "Branding",
    "Product Class",
    "Sub Product",
    "Packaging Size",
    ...
    "Forecast Non-Customer Invoiced Fixed Trade {{currency_symbol}} (LIVE)"
    as "CW Forecast Non-Customer Invoiced Fixed Trade {{currency_symbol}} (LIVE)",
    "Forecast PCOS {{currency_symbol}} (LIVE) Act" as "CW Forecast PCOS {{currency_symbol}} (LIVE) Act",
    "Forecast PCOS {{currency_symbol}} (LIVE) Std" as "CW Forecast PCOS {{currency_symbol}} (LIVE) Std",
    ...
    "CY Added Value Pack {{currency_symbol}}" as "CW CY Added Value Pack {{currency_symbol}}",
    "CY Total Customer-Invoiced Trade (Combo)"
    * -1 as "CW CY Total Customer-Invoiced Trade Spend (Combo)",
    ...
    "Cy PCOS Spend (Combo) Std" as "CW Cy PCOS Spend (Combo) Std",
    "Cy PCOS Spend (Combo) Act" as "CW Cy PCOS Spend (Combo) Act",
    ...
    "(Budget) Forecast NRR {{currency_symbol}} (Combo)" as "CW (Budget) Forecast NRR {{currency_symbol}} (Combo)",
    ...
    "Budget Variable Trade {{currency_symbol}} (Combo)" as "CW Budget Variable Trade {{currency_symbol}} (Combo)",
    ...
    "CY Total Variable Trade {{currency_symbol}} (Combo)" as "CW CY Total Variable Trade {{currency_symbol}} (Combo)",
    ...
    "Forecast PCOS {{currency_symbol}} (Combo) Act" as "CW Forecast PCOS {{currency_symbol}} (Combo) Act",
    "Forecast PCOS {{currency_symbol}} (Combo) Std" as "CW Forecast PCOS {{currency_symbol}} (Combo) Std",
    ...
    * -1 as "CW Budget Fixed Trade Spend",
    -1 * (
        (
    ...
    end as "CW Customer-Invoiced Variable Trade % vs Budget (Split)",
    case
        when
            nvl(
                (
    ...
    end as "CW Customer-Invoiced Variable Trade % vs Forecast (Split)",
    case
    ...
    end as "CW Customer-Invoiced Variable Trade % vs LY (Split)",
    ifnull(
    ...
    ifnull(
        ...
        0
    ) as "CW CY NRR {{currency_symbol}} (Combo)",
    final."CY Total Variable Trade {{currency_symbol}}"
    + - final."Forecast Total Variable Trade {{currency_symbol}}" * -1 as "CW Cy Variable Trade Spend",
    ifnull(
        "CY Permanent Discounts {{currency_symbol}}" + -1 * "Forecast Permanent Discounts {{currency_symbol}}", 0
    ...
    ) as "CW Forecast Fixed Trade Spend",
    (
        (
            (
                ((ifnull("Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0)))
    ...
    ) as "CW Forecast GCAT {{currency_symbol}} (Combo) Act",
    (
        (
            (
                ((ifnull("Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0)))
    ...
    ) as "CW Forecast GCAT {{currency_symbol}} (Combo) Std",
    ifnull(
        "Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0
    ...
    ifnull("Ly Net Sales (Combo)", 0) + ifnull(
        ...
        0
    ) as "CW LY NRR {{currency_symbol}} (Combo)",
    ifnull(
        "LY Total Customer-Invoiced Trade {{currency_symbol}}"
        + - "Forecast Total Customer-Invoiced Trade {{currency_symbol}}",
        0
    ...
    ) as "CW Abs Var Forecast GCAT Std",
    ...
    ) as "CW Abs Var Forecast GCAT Act",
    ifnull(
        ifnull(
            (
                ifnull("Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0)
    ...
    + ifnull(
        ...
        0
    ) as "CW CY GCAT {{currency_symbol}} (Combo) Act",
    ifnull(
        ifnull(
    ...
    + ifnull(
        ...
        0
    ) as "CW CY GCAT {{currency_symbol}} (Combo) Std",
    ...
    ) as "CW Forecast NRR {{currency_symbol}} (Combo)"
