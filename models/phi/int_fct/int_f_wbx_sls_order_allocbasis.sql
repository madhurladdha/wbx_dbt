...
aggregate as (
            select
        f.ship_source_customer_code,
        f.ship_customer_address_guid,
...
select
    ...
    -- -customer ship-to but not product information---
    sum(tot_shipped_kg_quantity) over (
        partition by ship_customer_address_guid, gl_month
    ) as tot_kg_ship_to,
    round(
    ...
    -- -item but no customer information---
    sum(tot_shipped_kg_quantity) over (partition by item_guid, gl_month) as tot_kg_item,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by item_guid, gl_month),
        8
    ) as perc_item,

    -- HJ 17 jul 2020,added for trade type and item
    -- -trade type and item information---
    sum(tot_shipped_kg_quantity) over (
        partition by trade_type_code, item_guid, gl_month
    ...
    sum(tot_shipped_kg_quantity) over (partition by gl_month) as tot_kg_month,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by gl_month),
        8
    ) as perc_month,
    -- -trade type but no product class or item information---
    sum(tot_shipped_kg_quantity) over (
        partition by trade_type_code, gl_month
    ) as tot_kg_trade_type,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by trade_type_code, gl_month),
        8
    ) as perc_trade_type,
    -- -product class but no item or customer information---
    sum(tot_shipped_kg_quantity) over (
        partition by product_class_code, gl_month
    ) as tot_kg_product_class,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by product_class_code, gl_month),
        8
    ) as perc_product_class,
    -- -trade type and product class but no item or customer ship-to----
    sum(tot_shipped_kg_quantity) over (
        partition by trade_type_code, product_class_code, gl_month
    ...
    sum(tot_shipped_kg_quantity) over (
        partition by branding_code, gl_month
    ) as tot_kg_branding_code,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by branding_code, gl_month),
        8
    ) as perc_branding_code,
    -- -market code from cost center but no trade type or customer or product level
    -- info---
    sum(tot_shipped_kg_quantity) over (
        partition by market_code, gl_month
    ) as tot_kg_market_code,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by market_code, gl_month),
        8
    ) as perc_market_code,
    -- -Sub-market code from cost center but no trade type or customer or product
    -- level info---
    sum(tot_shipped_kg_quantity) over (
        partition by sub_market_code, gl_month
    ) as tot_kg_submarket,
    round(
        tot_shipped_kg_quantity
        / sum(tot_shipped_kg_quantity) over (partition by sub_market_code, gl_month),
        8
    ) as perc_submarket,
    -- -market code from cost center but no trade type or custoemr and product class---
    sum(tot_shipped_kg_quantity) over (
        partition by market_code, product_class_code, gl_month
    ...
    -- class---
    sum(tot_shipped_kg_quantity) over (
        partition by sub_market_code, product_class_code, gl_month
    ...
    sum(tot_shipped_kg_quantity) over (
        partition by trade_type_code, branding_code, gl_month
    ...
    sum(tot_shipped_kg_quantity) over (
        partition by market_code, branding_code, gl_month
    ...
    sum(tot_shipped_kg_quantity) over (
        partition by sub_market_code, branding_code, gl_month
