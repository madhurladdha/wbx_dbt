{{ config(tags=["sales", "ibe_budget", "ibe_sls_budget", "ibe_sls_budget_fin","adhoc"]) }}

with
    stg as (select * from {{ ref("stg_f_wbx_sls_ibe_budget_fin") }}),
    item_cte as (
        ...
        from
            ...
            (
                select
                    ...
                    src_bud.budkgs / dim_date.split_factor * nvl(uom_kg.conversion_rate, 0) as budget_qty_ca,/*coverting kg in ca using conv factor Avinash 30-05-2024 */
                ...
                left join
                    ...
                    on uom_kg.item_guid = item_dim.item_guid
                    and uom_kg.from_uom = 'KG'
                    and uom_kg.to_uom = 'CA'
