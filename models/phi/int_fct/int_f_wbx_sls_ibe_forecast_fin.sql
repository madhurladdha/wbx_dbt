{{ config(tags=["sales", "ibe_forecast","adhoc","ibe"]) }}

with
stg as (select * from {{ ref("stg_f_wbx_sls_ibe_forecast_fin") }}),
...
snapshot_date_expr as (
    select to_date(convert_timezone('UTC', current_timestamp)) as snapshot_date
    from dual
),

/* stg_d_wtx_lkp_snapshot_date evalutes snapshot_date as (current_date-12).
As the automate job related to iberica forecast runs as 7 am cst model was picking previous day of run as snapshot date,which is not same as expected.
Hence changed model to use current_date instead of snapshot_date*/

snapshot_date as (
    select
        case
            when dayofweek(snapshot_date) = 6 then calendar_week_end_dt
            else (snapshot_date - 1)
        end as snapshot_date
    from snapshot_date_expr
    inner join dim_date_cte on
        dim_date_cte.calendar_date = snapshot_date_expr.snapshot_date
...
final as (
    select
        ...
        cast(ship_customer_address_guid as text(255))
            as ship_customer_address_guid,
        cast(
            substring(bill_source_customer_code, 1, 255) as text(255)
        ) as bill_source_customer_code,
        cast(bill_customer_address_guid as text(255))
        ...
        cast(sum(incentive_forced_amt) as number(38, 10))
            as incentive_forced_amt,
        ...
        cast(sum(net_value_grossup_amt) as number(38, 10))
            as net_value_grossup_amt,
        ...
        cast(sum(mif_customer_mktg_amt) as number(38, 10))
            as mif_customer_mktg_amt,
        ...
        cast(sum(fixed_annual_payment) as number(38, 10))
            as fixed_annual_payment,
        cast(sum(consumer_marketing) as number(38, 10)) as consumer_marketing, 
        cast(sum(rsatotal_new) as number(38, 10)) as rsatotal_new,
    ...
    from
        ...
        (
            select
                ...
                src_bud.comcde5d as source_item_identifier,
                item_dim.item_guid,
                ...
                nvl(src_bud.budqty * uom_kg.conversion_rate, 1)
                / dim_date.split_factor as budget_qty_kg,
                nvl(src_bud.budqty * uom_pl.conversion_rate, 1)
                / dim_date.split_factor as budget_qty_ul,
                nvl(src_bud.budqty * uom_ca.conversion_rate, 1)
                / dim_date.split_factor as budget_qty_prim,
                ...
                src_bud.wasteredval
                / dim_date.split_factor as waste_reduced_amt,
                ...
                src_bud.grossvaluetotal
                / dim_date.split_factor as gross_value_amt,
                src_bud.edlptotal / dim_date.split_factor as edlp_amt,
                src_bud.rsatotal / dim_date.split_factor as rsa_amt,
                src_bud.settlementtotal
                / dim_date.split_factor as settlement_amt,
                ...
                src_bud.backmargintotal
                / dim_date.split_factor as back_margin_amt,
                ...
                src_bud.boughtintotal
                / dim_date.split_factor as boughtin_cost_amt,
                src_bud.copackingtotal
                / dim_date.split_factor as copack_cost_amt,
                ...
                src_bud."MIF-CATEGORY"
                / dim_date.split_factor as mif_category_amt,
                ...
                src_bud.fixedannualpayment
                / dim_date.split_factor as fixed_annual_payment,
                src_bud.consumermarketing
                / dim_date.split_factor as consumer_marketing,
                 src_bud.rsatotal_new as rsatotal_new,
            ...
            from
                ...
                (
                    select
                        ...
                        sum("MIF-CUSTOMER MARKETING")
                            as "MIF-CUSTOMER MARKETING",
                        ...
                        sum(fixedannualpayment) as fixedannualpayment,
                        sum(consumermarketing) as consumermarketing,
                        sum(rsatotal_new) as rsatotal_new
            ...
            inner join
                ...
                on
                    dim_date.calendar_month_no
                    = cyr
                    || case
                        when cyrper < 10 then '0' || cyrper else to_char(cyrper)
                    end
            ...
            left join
                ...
                on
                    item_dim.source_item_identifier
                    = src_bud.comcde5d
            -- GET UOM CONVERSION RATE from Primary to KG
            left join
                     {{
            ...
            -- GET UOM CONVERSION RATE from Primary to CA
            left join
                     {{
                ent_dbt_package.lkp_uom(
                    "item_dim.item_guid",
            ...
            -- GET UOM CONVERSION RATE from Primary to PL
            left join
                     {{
                ent_dbt_package.lkp_uom(
                    "item_dim.item_guid",
            ...
            left join snapshot_date as snap on 1 = 1
        )
