...
with
    src as (select * from {{ ref("stg_f_wbx_fin_ap_voucher") }}),
    ...
    conv_rt as (
        select
            ...
            src.reference_txt,
            src.remark_txt,
            to_char(txn_uom_lkp.normalized_value) as transaction_uom,
        ...
        left join
            ...
            /* This condition filters out unwanted document types to align w/ how IICS behaves. */
        where document_type_lkp.normalized_value is not null
    )
...
select
    ...
    cast(substring(document_pay_item_ext, 1, 255) as text(255)) as document_pay_item_ext,
    ...
    cast(
        coalesce(base_taxable_amt * phi_conv_rt, 0) as number(38, 10)
    ) as phi_taxable_amt,

    cast(coalesce(base_tax_amt * phi_conv_rt, 0) as number(38, 10)) as phi_tax_amt,

    cast(
        coalesce(base_gross_amt * pcomp_conv_rt, 0) as number(38, 10)
    ) as pcomp_gross_amt,

    cast(
        coalesce(base_open_amt * pcomp_conv_rt, 0) as number(38, 10)
    ) as pcomp_open_amt,
    ...
    cast(
        coalesce(base_taxable_amt * pcomp_conv_rt, 0) as number(38, 10)
