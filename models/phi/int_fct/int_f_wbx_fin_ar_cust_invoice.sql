...
with
    src as (select * from {{ ref("stg_f_wbx_fin_ar_cust_invoice") }}),
    ...
    conv_rt as (
        ...
        /*
            ...
            */
            ...
            {{
                dbt_utils.surrogate_key(
                    [
                        ...
                        "'CUSTOMER_MAIN'","src.document_company"
                    ]
                )
            ...
            {{
                dbt_utils.surrogate_key(
                    ["src.source_system", "src.source_item_identifier"]
                )
            }} as item_guid,
            {{
                dbt_utils.surrogate_key(
            ...
            src.net_due_date,
            src.original_document_number,
            src.original_document_type,
            src.original_document_company,
            src.original_document_pay_item,
        ...
        left join
            ...
            and dim_wbx_account.source_account_identifier = src.source_account_identifier
...
select
    ...
    cast(
        substring(original_document_number, 1, 255) as text(255)
    ) as original_document_number,

    cast(
        substring(original_document_type, 1, 255) as text(255)
    ) as original_document_type,

    cast(
        substring(original_document_company, 1, 255) as text(255)
    ) as original_document_company,

    cast(
        substring(original_document_pay_item, 1, 255) as text(255)
    ...
    cast(
        substring(sales_document_suffix, 1, 255) as text(255)
