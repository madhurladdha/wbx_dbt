...
with
    stg_f_wbx_sls_budget_promo as (
        select * from {{ ref("stg_f_wbx_sls_budget_promo") }}
    ),
    stage as (
        select
            source_system,
            ...
            trunc(current_date, 'DD') as snapshot_date,
            reportingsku_idx,
            ...
            promo_code
        from stg_f_wbx_sls_budget_promo
        where
    ...
    final as (
        select
            ...
            {{ dbt_utils.surrogate_key(["stg.source_system", "stg.promo_idx"]) }}
            as promo_guid,
            cust_idx,
            plan_source_customer_code,
            ...
            stg.scen_name,
            stg.scen_code,
            -- scen_lkp.scenario_guid,
            {{ dbt_utils.surrogate_key(["stg.source_system", "stg.scen_idx"]) }}
            as scenario_guid,
            frozen_forecast,
            si_b_vol_cse,
            si_b_vol_sgl,
        ...
        left join
            {{
                ent_dbt_package.lkp_uom(
                    "item_lkp.item_guid", "'KG'", "'CA'", "uom_kg_ca_lkp"
                )
            }}
        left join
            {{
                ent_dbt_package.lkp_uom(
                    "item_lkp.item_guid", "'CA'", "'KG'", "uom_ca_kg_lkp"
                )
            }}
        left join
            {{
                ent_dbt_package.lkp_uom(
                    "item_lkp.item_guid", "'CA'", "'PL'", "uom_ca_pl_lkp"
                )
            }}
    )
