...
with
    ...
    uber_cte as (
        ...
        where
            calendar_date in (
                ...
                from dim_date_cte
                where
                    to_date(fiscal_quarter_start_dt) in (
                        select max(delineation_date)
                        from scen_xref
                        where frozen_forecast like 'F%'
                    )
            )
            and frozen_forecast = (
                select distinct frozen_forecast
                from scen_xref
                where delineation_date in (select max(delineation_date) from scen_xref)
            )
        union
        select *
        from uber
    ...
    pre_processing as (
        select
            ...
            sum(
                nvl(ub.fcf_tot_vol_ca * itm_ext.consumer_units_in_trade_units, 0)
...
select
    le_unique_key as le_unique_key,
    "Branding",
    "Product Class",
    "Sub Product",
    "Packaging Size",
    ...
    "CY Added Value Pack {{currency_symbol}}" as "LE CY Added Value Pack {{currency_symbol}}",
    "CY Total Customer-Invoiced Trade (Combo)"
    * -1 as "LE CY Total Customer-Invoiced Trade Spend (Combo)",
    ...
    "Cy PCOS Spend (Combo) Std" as "LE Cy PCOS Spend (Combo) Std",
    "Cy PCOS Spend (Combo) Act" as "LE Cy PCOS Spend (Combo) Act",
    ...
    "(Budget) Forecast NRR {{currency_symbol}} (Combo)" as "LE (Budget) Forecast NRR {{currency_symbol}} (Combo)",
    ...
    "Budget Variable Trade {{currency_symbol}} (Combo)" as "LE Budget Variable Trade {{currency_symbol}} (Combo)",
    ...
    "CY Total Variable Trade {{currency_symbol}} (Combo)" as "LE CY Total Variable Trade {{currency_symbol}} (Combo)",
    ...
    "Forecast PCOS {{currency_symbol}} (Combo) Act" as "LE Forecast PCOS {{currency_symbol}} (Combo) Act",
    "Forecast PCOS {{currency_symbol}} (Combo) Std" as "LE Forecast PCOS {{currency_symbol}} (Combo) Std",
    ...
    * -1 as "LE Budget Fixed Trade Spend",
    -1 * (
        (
    ...
    end as "LE Customer-Invoiced Variable Trade % vs Budget (Split)",
    case
        when
            nvl(
                (
    ...
    end as "LE Customer-Invoiced Variable Trade % vs Forecast (Split)",
    case
    ...
    end as "LE Customer-Invoiced Variable Trade % vs LY (Split)",
    ifnull(
    ...
    ifnull(
        ...
        0
    ) as "LE CY NRR {{currency_symbol}} (Combo)",
    final."CY Total Variable Trade {{currency_symbol}}"
    + - final."Forecast Total Variable Trade {{currency_symbol}}" * -1 as "LE Cy Variable Trade Spend",
    ifnull(
        "CY Permanent Discounts {{currency_symbol}}" + -1 * "Forecast Permanent Discounts {{currency_symbol}}", 0
    ...
    ) as "LE Forecast Fixed Trade Spend",
    (
        (
            (
                ((ifnull("Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0)))
    ...
    ) as "LE Forecast GCAT {{currency_symbol}} (Combo) Act",
    (
        (
            (
                ((ifnull("Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0)))
    ...
    ) as "LE Forecast GCAT {{currency_symbol}} (Combo) Std",
    ifnull(
        "Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0
    ...
    ifnull("Ly Net Sales (Combo)", 0) + ifnull(
        ...
        0
    ) as "LE LY NRR {{currency_symbol}} (Combo)",
    ifnull(
        "LY Total Customer-Invoiced Trade {{currency_symbol}}"
        + - "Forecast Total Customer-Invoiced Trade {{currency_symbol}}",
        0
    ...
    ) as "LE Abs Var Forecast GCAT Std",
    ...
    ) as "LE Abs Var Forecast GCAT Act",
    ifnull(
        ifnull(
            (
                ifnull("Cy Invoice Gross Amount {{currency_symbol}}" + "Forecast Gross Amount {{currency_symbol}}", 0)
    ...
    + ifnull(
        ...
        0
    ) as "LE CY GCAT {{currency_symbol}} (Combo) Act",
    ifnull(
        ifnull(
    ...
    + ifnull(
        ...
        0
    ) as "LE CY GCAT {{currency_symbol}} (Combo) Std",
    ...
    ) as "LE Forecast NRR {{currency_symbol}} (Combo)"
