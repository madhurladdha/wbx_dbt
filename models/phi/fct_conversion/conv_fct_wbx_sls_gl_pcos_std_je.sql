...
with old_fct as (
    select *
    from {{ source("WBX_PROD_FACT", "fct_wbx_sls_gl_pcos_std_je") }}
    where
...
select
    ...
    cast(substring(item_guid, 1, 255) as text(255))
        as item_guid,
    cast(substring(ship_source_customer_code, 1, 255) as text(255))
        as ship_source_customer_code,
    cast(substring(bill_source_customer_code, 1, 255) as text(255))
        as bill_source_customer_code,
    cast(substring(ship_customer_addr_number_guid, 1, 255) as text(255))
        as ship_customer_addr_number_guid,
    cast(substring(bill_customer_addr_number_guid, 1, 255) as text(255))
        as bill_customer_addr_number_guid,
