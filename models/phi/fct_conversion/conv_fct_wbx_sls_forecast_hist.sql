...
WITH 
old_fct AS (
    SELECT * FROM {{source('FACTS_FOR_COMPARE','sls_wtx_forecast_hist')}} 
...
converted_fct as (
    ...
    {{dbt_utils.surrogate_key(["source_system","ltrim(rtrim(source_item_identifier))",])}} as item_guid,
    ...
    0 as plan_customer_addr_number_guid,  /* always zero since this is a planning customer  */
    ...
    {{dbt_utils.surrogate_key(["source_system","ltrim(rtrim(scen_code))",])}} as scenario_guid,
    ...
    cast(nvl(retail_tot_vol_ca,0) as number(38,10) ) as retail_tot_vol_ca  ,
    ...
    cast(nvl(retail_tot_vol_ul,0) as number(38,10) ) as retail_tot_vol_ul  ,
